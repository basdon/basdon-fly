...
$page = get_page();
$db_querycount += 2;
$tracthreads = $db->query('SELECT id,updated,released,state,severity,summary FROM tract WHERE visibility&'.$usergroups.' OR op='.$userid.' ORDER BY state<4 DESC, id DESC LIMIT 100 OFFSET '.(($page - 1) * 100));
